...
//       populateCommission();
    ...
    public void populateCommission(Order order){
            int com = 0;
        ...
        txtCommission.setText(String.valueOf(com));
    }
...
//                {
                    for (OrderItem oi : order.getOrder()) //judge if there is a same product in the cart or not
                    {
                        if (oi.getMarketOffer().getProduct() == selectedProduct) {//same product
...
//                {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if(order.getOrder().size()>0)
        {
            ...
            order.setCustomer(c);
            //analyze the total number sold of the product
            for(OrderItem oi : order.getOrder())
            {
                if(oi.getSellingPrice()>oi.getMarketOffer().getTargetPrice())
                {
                    oi.getMarketOffer().getProduct().setSoldNum(oi.getQuantity());
                }
            }
